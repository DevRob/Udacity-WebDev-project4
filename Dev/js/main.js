...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    /**
      ...
      replaced "document.querySelectorAll(".randomPizzaContainer");" and
    ...
    */
    ...
    for (var i = 0, len = pizzaBox.length; i < len; i++) {
...
function updatePositions() {
  ...
  /**
    moved out (document.body.scrollTop / 1250) from loop and
    saved items.lenght in a local variable.
    used style.transform methode for smooth rendering.
    moved and combined "elem.basicLeft = (i % cols) * s;" from the "mover" generator
    replaced document.querySelectorAll();
  */
  var topScroll = window.scrollY / 1250;
  var items = document.getElementsByClassName('mover');
  for (var i = 0, len = items.length; i < len; i++) {
    items[i].style.transform = 'translate(' + ((i % cols) * s + 100 * Math.sin(topScroll + (i % 5))) + 'px, 0px)';
  }
...
/**
  Moved style properties out from loop to style.css
    height : 100px;
    width : 73.333px;
  moved "elem.basicLeft = (i % cols) * s;" up to style.transform
  moved out elem variable declaration from foor loop
*/
